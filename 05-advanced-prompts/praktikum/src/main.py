"""Praktikum 05 — Advanced Prompts.

Lengkapi keempat fungsi bertanda TODO, lalu jalankan:
    python src/main.py
Demo yang TODO-nya belum selesai otomatis dilewati.
"""
import os
...
# ─────────────────────────────────────────────────────────────
# TODO 1: generate dengan kontrol temperature
# ─────────────────────────────────────────────────────────────
def generate(prompt: str, temperature: float = None) -> str:
    """Kirim prompt ke /api/generate; bila `temperature` diisi, sertakan.

    Petunjuk:
    1. Buat body = {"model": MODEL, "prompt": prompt, "stream": False}
    2. Jika temperature is not None, tambahkan:
       body["options"] = {"temperature": temperature}
    3. POST ke f"{BASE_URL}/api/generate", raise_for_status(),
       kembalikan r.json()["response"].
    """
    raise NotImplementedError("TODO 1 (generate) belum dikerjakan")


# ─────────────────────────────────────────────────────────────
# TODO 2: Chain-of-Thought — fungsi murni penyusun prompt
# ─────────────────────────────────────────────────────────────
def prompt_cot(soal: str) -> str:
    """Bungkus `soal` menjadi prompt chain-of-thought.

    Format wajib (soal dulu, lalu instruksi berpikir):
        {soal}

        Mari berpikir langkah demi langkah. Tulis penalaranmu,
        lalu akhiri dengan baris "JAWABAN: <angka/jawaban akhir>".
    """
    raise NotImplementedError("TODO 2 (prompt_cot) belum dikerjakan")


# ─────────────────────────────────────────────────────────────
# TODO 3: Self-consistency — tanya berulang, bandingkan jawaban
# ─────────────────────────────────────────────────────────────
def self_consistency(soal: str, n: int = 3) -> list:
    """Kembalikan list berisi `n` jawaban model untuk soal yang sama.

    Petunjuk: panggil generate(prompt_cot(soal), temperature=0.9)
    sebanyak n kali, kumpulkan hasilnya dalam list.
    """
    raise NotImplementedError("TODO 3 (self_consistency) belum dikerjakan")


# ─────────────────────────────────────────────────────────────
# TODO 4: Self-refine — fungsi murni penyusun prompt kritik
# ─────────────────────────────────────────────────────────────
def prompt_refine(jawaban: str, instruksi_awal: str) -> str:
    """Susun prompt yang meminta model MENGKRITIK lalu MEMPERBAIKI jawaban.

    Prompt harus memuat `instruksi_awal` dan `jawaban`, misalnya:
        Tugas awal: {instruksi_awal}
        Jawaban sebelumnya: {jawaban}
        Beri 2 kritik singkat atas jawaban itu, lalu tulis versi
        yang sudah diperbaiki.
    """
    raise NotImplementedError("TODO 4 (prompt_refine) belum dikerjakan")
